...
- `coverage-3.10` → `src/coverage.xml`
- `coverage-3.11` → `src/coverage.xml`
...
- Smoke-build the Docker image (no push)
...
```bash
docker build -f ./docker/python/Dockerfile -t iot-hub/python:local .
```
...
2. **Avoid `sleep()`**: Use proper synchronization or mocking instead
3. **Pin random seeds**: If using random data, set `Faker.seed()` in conftest
4. **Refresh from DB**: After creating objects with DB defaults, call `refresh_from_db()`
